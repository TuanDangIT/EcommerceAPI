...
namespace Ecommerce.Modules.Inventory.Application.Inventory.Services
{
    internal class EventMapper : IEventMapper
    {
        public IMessage Map(IDomainEvent @event)
            => @event switch
            {
                Domain.Inventory.Events.ProductsListed e => new Events.ProductsListed(e.Products.Select(p => new { p.Id, p.SKU, p.Name, p.Price, p.Quantity,ImagePathUrl = p.Images.First().ImageUrlPath })),
